...
extern const uint8_t motoronCrcTable[256];
...
public:
    ...
    void getFirmwareVersion(uint16_t *productId, uint16_t *firmwareVersion)
    {
        ...
        {
            *productId = response[0] | (response[1] << 8);
        }
        if (firmwareVersion != nullptr)
        {
            *firmwareVersion = response[2] | (response[3] << 8);
        }
    }
    ...
    void enableCrc()
    {
        setProtocolOptions(protocolOptions | (1 << MOTORON_PROTOCOL_OPTION_CRC_FOR_COMMANDS) | (1 << MOTORON_PROTOCOL_OPTION_CRC_FOR_RESPONSES));
    }
    ...
    {
        setProtocolOptions(protocolOptions | (1 << MOTORON_PROTOCOL_OPTION_CRC_FOR_COMMANDS));
    }
    ...
    {
        setProtocolOptions(protocolOptions | (1 << MOTORON_PROTOCOL_OPTION_CRC_FOR_RESPONSES));
    }
    ...
    {
        setProtocolOptions(protocolOptions | (1 << MOTORON_PROTOCOL_OPTION_I2C_GENERAL_CALL));
    }
    ...
    {
        if (baud < MOTORON_MIN_BAUD_RATE)
        {
            baud = MOTORON_MIN_BAUD_RATE;
        }
        if (baud > MOTORON_MAX_BAUD_RATE)
        {
            baud = MOTORON_MAX_BAUD_RATE;
        }
                                ...
                                uint8_t init = 0)
    {
        ...
        {
            crc = motoronCrcTable[crc ^ buffer[i]];
...
private:
    ...
    void sendCommandCore(uint8_t length, const uint8_t *cmd, bool sendCrc) override
    {
        ...
        if (sendCrc)
        {
            buf[length] = calculateCrc(length, cmd);
        }
        int ret = i2c_write_blocking(i2c_default, address, buf, length + (sendCrc ? 1 : 0), false);
        if (ret != length + (sendCrc ? 1 : 0))
        {
            lastError = 4;
        }
        else
    ...
    void readResponse(uint8_t length, uint8_t *response) override
    {
        ...
        uint8_t buf[1024];
        if (length + (crcEnabled ? 1 : 0) > sizeof(buf))
        {
            memset(response, 0, length);
            lastError = 51;
            return;
        }
        int ret = i2c_read_blocking(i2c_default, address, buf, length + (crcEnabled ? 1 : 0), false);
        if (ret != length + (crcEnabled ? 1 : 0))
        {
            memset(response, 0, length);
            lastError = 50;
            return;
        ...
        if (crcEnabled && buf[length] != calculateCrc(length, response))
        {
            lastError = 51;
            return;
        }
